...
static int inline filter_compare(switch_event_t *evt, kazoo_filter_ptr filter, kazoo_logging_ptr logging)
{
	...
	case FILTER_COMPARE_EXISTS:
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, logging->levels->trace_log_level,
						  "Profile[%s] event %s checking if %s exists: %s\n", logging->profile_name,
						  logging->event_name, filter->name, hasValue ? "true" : "false");
		break;
	...
	case FILTER_COMPARE_VALUE:
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, logging->levels->trace_log_level,
						  "Profile[%s] event %s compare value %s to %s: %s == %s => %s\n", logging->profile_name,
						  logging->event_name, filter->name, filter->value, value, filter->value,
						  hasValue ? "true" : "false");
		break;
	...
	case FILTER_COMPARE_FIELD:
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, logging->levels->trace_log_level,
						  "Profile[%s] event %s compare field %s to %s: %s == %s => %s\n", logging->profile_name,
						  logging->event_name, filter->name, filter->value, value,
						  switch_event_get_header_nil(evt, filter->value), hasValue ? "true" : "false");
		break;
...
static switch_status_t kazoo_event_init_json(kazoo_fields_ptr fields1, kazoo_fields_ptr fields2, switch_event_t *evt,
											 cJSON **clone)
{
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	if ((fields2 && fields2->verbose) || (fields1 && fields1->verbose) || ((!fields2) && (!fields1))) {
...
static cJSON *kazoo_event_json_value(kazoo_json_field_type type, const char *value)
{
	...
	case JSON_RAW:
		...
		if (!item) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Failed to decode JSON\n");
	...
	case FIELD_FIRST_OF:
		...
		for (n = 0; n < field->list.size; n++) {
			if (*field->list.value[n] == '#') {
				item =
					kazoo_event_add_json_value(dst, field, field->as ? field->as : field->name, ++field->list.value[n]);
				break;
			} else {
				...
				if (header) {
					if (header->idx) {
						...
						item =
							kazoo_event_add_json_value(dst, field, field->as ? field->as : field->name, header->value);
					}
		...
		for (header = src->headers; header; header = header->next) {
			if (!strncmp(header->name, field->name, strlen(field->name))) {
				if (header->idx) {
					...
					kazoo_cJSON_AddItemToObject(
						dst, field->exclude_prefix ? header->name + strlen(field->name) : header->name, array);
				} else {
					kazoo_event_add_json_value(
						dst, field, field->exclude_prefix ? header->name + strlen(field->name) : header->name,
						header->value);
				}
			}
		}
...
static switch_status_t kazoo_event_add_fields_to_json(kazoo_logging_ptr logging, cJSON *dst, switch_event_t *src,
													  kazoo_field_ptr field)
{
		...
		if (field->in_type == FIELD_REFERENCE) {
			if (field->ref) {
				if ((filter = filter_event(src, field->ref->filter, logging)) != NULL) {
					switch_log_printf(SWITCH_CHANNEL_LOG, logging->levels->filtered_field_log_level,
									  "profile[%s] event %s, referenced field %s filtered by settings %s: %s\n",
									  logging->profile_name, logging->event_name, field->ref->name, filter->name,
									  filter->value);
				} else {
				...
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
								  "profile[%s] event %s, referenced field %s not found\n", logging->profile_name,
								  logging->event_name, field->name);
			}
		} else {
			if ((filter = filter_event(src, field->filter, logging)) != NULL) {
				switch_log_printf(SWITCH_CHANNEL_LOG, logging->levels->filtered_field_log_level,
								  "profile[%s] event %s, field %s filtered by settings %s: %s\n", logging->profile_name,
								  logging->event_name, field->name, filter->name, filter->value);
			} else {
				...
				if (field->children && item != NULL) {
					...
					kazoo_event_add_fields_to_json(logging, field->out_type == JSON_OBJECT ? item : dst, src,
												   field->children->head);
				}
			}
		}
...
kazoo_message_ptr kazoo_message_create_event(switch_event_t *evt, kazoo_event_ptr event,
											 kazoo_event_profile_ptr profile)
{
	...
	if (message == NULL) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
						  "error allocating memory for serializing event to json\n");
		return NULL;
	}
	...
	if (profile->filter) {
		...
		if ((filtered = filter_event(evt, profile->filter, &logging)) != NULL) {
			switch_log_printf(SWITCH_CHANNEL_LOG, logging.levels->filtered_event_log_level,
							  "profile[%s] event %s filtered by profile settings %s: %s\n", logging.profile_name,
	...
	if (event && event->filter) {
		if ((filtered = filter_event(evt, event->filter, &logging)) != NULL) {
			switch_log_printf(SWITCH_CHANNEL_LOG, logging.levels->filtered_event_log_level,
							  "profile[%s] event %s filtered by event settings %s: %s\n", logging.profile_name,
							  logging.event_name, filtered->name, filtered->value);
			kazoo_message_destroy(&message);
			return NULL;
		}
