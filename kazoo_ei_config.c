...
#define KAZOO_DECLARE_GLOBAL_STRING_FUNC(fname, vname)                                                                 \
	static void __attribute__((__unused__)) fname(const char *string)                                                  \
	{                                                                                                                  \
		if (!string)                                                                                                   \
			return;                                                                                                    \
		if (vname) {                                                                                                   \
	...
	if (!stat(filename, &buf)) {
		if ((buf.st_mode & S_IRWXG) || (buf.st_mode & S_IRWXO)) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
							  "Cookie file %s must only be accessible by owner only\n", filename);
			return 2;
		}
		if (buf.st_size > MAXATOMLEN) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
							  "Cookie file %s contains a cookie larger than the maximum atom size of %d\n", filename,
							  MAXATOMLEN);
			return 2;
		}
		fd = open(filename, O_RDONLY);
		if (fd < 1) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Failed to open cookie file %s: %d\n", filename,
							  errno);
			return 2;
		}

		if (read(fd, cookie, MAXATOMLEN) < 1) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Failed to read cookie file %s: %d\n", filename,
							  errno);
		}
...
switch_status_t kazoo_ei_config(switch_xml_t cfg)
{
		...
		for (param = switch_xml_child(child, "param"); param; param = param->next) {
			...
			if (!strcmp(var, "listen-ip")) {
				...
				if (read_cookie_from_file(val) == 1) {
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Failed to read cookie from %s\n", val);
				}
	...
	if ((child = switch_xml_child(cfg, "tweaks"))) {
		...
		if (default_tweaks && !zstr(default_tweaks)) {
			...
			for (i = 0; i < KZ_TWEAK_MAX; i++)
				kazoo_globals.tweaks[i] = v;
		}
	...
	kazoo_globals.profile_vars_prefixes =
		switch_core_alloc(kazoo_globals.pool, sizeof(char *) * KZ_MAX_SEPARATE_STRINGS);
...
switch_status_t kazoo_load_config()
{
	...
	if (!(xml = switch_xml_open_cfg(cf, &cfg, NULL))) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
						  "Failed to load configuration (failed to open configuration file %s)\n", cf);
...
switch_status_t kazoo_config_events(kazoo_config_ptr definitions, switch_memory_pool_t *pool, switch_xml_t cfg,
...
switch_status_t kazoo_config_fetch_handler(kazoo_config_ptr definitions, kazoo_config_ptr root, switch_xml_t cfg,
										   kazoo_fetch_profile_ptr *ptr)
{
	...
	if (switch_core_new_memory_pool(&pool) != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
						  "Failed to create a new sub memory pool for profile: %s\n", name);
...
switch_status_t kazoo_config_event_handler(kazoo_config_ptr definitions, kazoo_config_ptr root, switch_xml_t cfg,
										   kazoo_event_profile_ptr *ptr)
{
