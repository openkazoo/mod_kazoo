...
static const char *LOG_LEVEL_NAMES[] = {"SWITCH_LOG_DEBUG10", "SWITCH_LOG_DEBUG9",	"SWITCH_LOG_DEBUG8",
										"SWITCH_LOG_DEBUG7",  "SWITCH_LOG_DEBUG6",	"SWITCH_LOG_DEBUG5",
										"SWITCH_LOG_DEBUG4",  "SWITCH_LOG_DEBUG3",	"SWITCH_LOG_DEBUG2",
										"SWITCH_LOG_DEBUG1",  "SWITCH_LOG_DEBUG",	"SWITCH_LOG_INFO",
										"SWITCH_LOG_NOTICE",  "SWITCH_LOG_WARNING", "SWITCH_LOG_ERROR",
										"SWITCH_LOG_CRIT",	  "SWITCH_LOG_ALERT",	"SWITCH_LOG_CONSOLE",
										"SWITCH_LOG_INVALID", "SWITCH_LOG_UNINIT",	NULL};

static const switch_log_level_t LOG_LEVEL_VALUES[] = {
	SWITCH_LOG_DEBUG10, SWITCH_LOG_DEBUG9, SWITCH_LOG_DEBUG8,  SWITCH_LOG_DEBUG7,  SWITCH_LOG_DEBUG6,
	SWITCH_LOG_DEBUG5,	SWITCH_LOG_DEBUG4, SWITCH_LOG_DEBUG3,  SWITCH_LOG_DEBUG2,  SWITCH_LOG_DEBUG1,
	SWITCH_LOG_DEBUG,	SWITCH_LOG_INFO,   SWITCH_LOG_NOTICE,  SWITCH_LOG_WARNING, SWITCH_LOG_ERROR,
	SWITCH_LOG_CRIT,	SWITCH_LOG_ALERT,  SWITCH_LOG_CONSOLE, SWITCH_LOG_INVALID, SWITCH_LOG_UNINIT};
...
switch_status_t kazoo_config_field(kazoo_config_ptr definitions, switch_memory_pool_t *pool, switch_xml_t cfg,
								   kazoo_field_ptr *ptr)
{
	...
	if (cur->children != NULL && (cur->in_type == FIELD_STATIC) && (cur->out_type == JSON_NONE)) {
		cur->out_type = JSON_OBJECT;
	}
...
switch_status_t kazoo_config_fields_loop(kazoo_config_ptr definitions, switch_memory_pool_t *pool, switch_xml_t cfg,
										 kazoo_field_ptr *ptr)
{
	switch_xml_t field;
...
switch_status_t kazoo_config_fields(kazoo_config_ptr definitions, switch_memory_pool_t *pool, switch_xml_t cfg,
									kazoo_fields_ptr *ptr)
{
	switch_xml_t fields;
...
kazoo_config_ptr kazoo_config_event_handlers(kazoo_config_ptr definitions, switch_xml_t cfg)
{
	...
	if (switch_core_new_memory_pool(&pool) != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
						  "Failed to create a new sub memory pool for producers\n");
		return NULL;
	}
	...
	if ((xml_profiles = switch_xml_child(cfg, "event-handlers"))) {
		if ((xml_profile = switch_xml_child(xml_profiles, "profile"))) {
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
							  "Failed to locate a event-handler profile for Kazoo\n");
		}
	} else {
		destroy_config(&profiles);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
						  "Failed to locate event-handlers section for Kazoo, using default\n");
	}
...
kazoo_config_ptr kazoo_config_fetch_handlers(kazoo_config_ptr definitions, switch_xml_t cfg)
{
	...
	if ((xml_profiles = switch_xml_child(cfg, "fetch-handlers"))) {
		if ((xml_profile = switch_xml_child(xml_profiles, "profile"))) {
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
							  "Failed to locate a fetch-handler profile for Kazoo\n");
		}
	} else {
		destroy_config(&profiles);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
						  "Failed to locate fetch-handlers section for Kazoo, using default\n");
	}
...
switch_status_t kazoo_config_definition(kazoo_config_ptr root, switch_xml_t cfg)
{
	...
	if (zstr(name)) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
						  "Failed to load kazoo profile, check definition missing name attr\n");
	...
	if (switch_core_hash_insert(root->hash, name, (void *)definition) != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
						  "Failed to insert new definition [%s] into Kazoo definitions hash\n", name);
		return SWITCH_STATUS_GENERR;
	}
...
kazoo_config_ptr kazoo_config_definitions(switch_xml_t cfg)
{
	...
	if (switch_core_new_memory_pool(&pool) != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
						  "Failed to create a new sub memory pool for definitions\n");
		return NULL;
	}
	...
	if ((xml_definitions = switch_xml_child(cfg, "definitions"))) {
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
						  "Failed to locate definitions section for Kazoo, using default\n");
	}
